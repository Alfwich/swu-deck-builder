...
- The hosted OpenAI assistant and local Codex assistant accept PNG, JPEG, and WebP images up to 10 MB. On mobile, the photo control can open the rear-facing camera; gallery selection, paste, and drag-and-drop remain available where the browser supports them. Uploads are bound to the active assistant session, staged as temporary files, and removed after the request or after five minutes if unused.
...
- `/api/features` exposes AI controls only to currently authorized IPs. The unlinked `/enable` route accepts the shared password and grants a fixed in-memory lease for the proxy-resolved public IP using `AGENT_ACCESS_LEASE_TTL_MS`. Reauthentication replaces it with a fresh lease, and server restarts clear all leases. Use the gate only over HTTPS.
