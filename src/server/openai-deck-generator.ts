...
import { applyAgentOperations } from './agent-operations.js'
import {
  COMPACT_AGENT_CARD_GROUPS_INSTRUCTIONS,
  serializeAgentChatTurn,
  serializeAgentDeckPayload,
} from './agent-deck-payload.js'
import { ensureAgentCatalogArtifact } from './catalog.js'
...
Collection-change context may include both the latest chronological collection revisions and net additions or removals since each deck's most recent content change. Use chronological events to answer what the user just added, acquired, removed, or changed; do not cancel one revision against another. Use per-deck net additions to identify cards newly available relative to a deck, and verify every recommendation against the authoritative current collection and catalog. When asked whether recent acquisitions improve any deck, evaluate every supplied deck snapshot and identify the strongest concrete candidates from currently owned recent additions. Do not describe a card as newly available relative to a deck when historyAvailable is false or it is absent from that deck's additions.
