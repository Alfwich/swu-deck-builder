...
export function ImportDeckDialog({ source, setSource, error, onClose, onSubmit }) {
  useEffect(() => {
...
export function ImportDatabaseDialog({ backup, fileName, onClose, onConfirm }) {
  useEffect(() => {
    function handleKeyDown(event) {
      if (event.key === 'Escape') {
        onClose()
...
export function PendingDatabaseImportDialog({ pending, onClose, onConfirm }) {
