...
export function agentImageSelectionTitle(
...
export function validateAgentImageFile(file: File | null | undefined) {
...
export function droppedImageFiles(dataTransfer: DataTransfer | null) {
...
export function agentImageQueuePrompt(
  prompt: string,
  position: number,
  total: number,
) {
...
export function shouldPresentAgentImageProposal(position: number, total: number) {
  return total <= 1 || position === total - 1
}

export function agentImageDisplayName(file: Pick<File, 'name'> | null) {
