...
import { DictationControl } from './dictation-control.js'
import {
  AgentChatProposal,
  AgentMessageText,
} from './agent-messages.js'
import { useMediaQuery } from '../shared/use-media-query.js'
import type { PreviewEvent } from './use-agent-card-preview.js'
import type { QueuedAgentImage } from './use-agent-images.js'
import type { AgentMessage } from '../types/assistant.js'
import type { DeckCard, ReadonlyCardReferenceMap } from '../types/catalog.js'

type AgentChatStatus = 'idle' | 'loading' | 'error'
type ResizeDirection = 'height' | 'width' | 'corner'
...
export function AgentChatPanel({
  ...
  history: string[]
  imageAttachments: QueuedAgentImage[]
  ...
  useLayoutEffect(() => {
    ...
    const bounds = {
      ...
      ...(typeof topBarBottom === 'number' && Number.isFinite(topBarBottom)
  ...
  function resizePanelToPointer(clientY: number) {
    const panel = panelRef.current
    if (!panel) return
  ...
  function handleResizePointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    resizePointerOffsetRef.current =
      event.clientY - panel.getBoundingClientRect().top
  ...
  function resizePanelWidthToPointer(clientX: number) {
    const panel = panelRef.current
    if (!panel) return
  ...
  function handleWidthResizePointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    resizeWidthPointerOffsetRef.current =
      panel.getBoundingClientRect().right - event.clientX
    event.currentTarget.setPointerCapture(event.pointerId)
    setAgentChatSize(getAgentChatSizeAfterResize)
    setResizeDirection('width')
  }

  function handleWidthResizePointerMove(event: ReactPointerEvent<HTMLDivElement>) {
    if (!event.currentTarget.hasPointerCapture(event.pointerId)) return
    resizePanelWidthToPointer(event.clientX)
  }

  function handleCornerResizePointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    if (event.button !== 0 || isMobileLayout) return
  ...
  function handleCornerResizePointerMove(event: ReactPointerEvent<HTMLDivElement>) {
    if (!event.currentTarget.hasPointerCapture(event.pointerId)) return
    resizePanelToPointer(event.clientY)
    resizePanelWidthToPointer(event.clientX)
  }
  ...
  function handleWidthResizeKeyDown(event: ReactKeyboardEvent<HTMLDivElement>) {
  ...
  function handleCornerResizeKeyDown(event: ReactKeyboardEvent<HTMLDivElement>) {
  ...
  function handleImageDragEnter(event: ReactDragEvent<HTMLFormElement>) {
  ...
  function handleImageDragOver(event: ReactDragEvent<HTMLFormElement>) {
    if (!imageAttachmentsAvailable || !event.dataTransfer.types.includes('Files')) {
      return
    }
    event.preventDefault()
  ...
  function handleImageDragLeave(event: ReactDragEvent<HTMLFormElement>) {
    if (!imageAttachmentsAvailable || !event.dataTransfer.types.includes('Files')) {
      return
    }
    event.preventDefault()
  ...
  function handlePanelSizeChange(nextIsCompact: boolean) {
  ...
  const accessFeatures = accessNotice?.features ?? []
