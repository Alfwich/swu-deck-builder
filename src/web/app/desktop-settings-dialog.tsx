...
function reasoningOptions(provider: DesktopAgentProvider) {
...
function connectionDescription(effective: EffectiveDesktopSettings | null) {
  if (!effective?.enabled) {
    return 'AI Deck Assistant is disabled.'
  }
  if (effective.available) {
    const providerLabel = effective.provider
      ? PROVIDER_LABELS[effective.provider] ?? 'Local CLI'
      : 'Local CLI'
    return `${providerLabel} is ready.`
  }
...
export function DesktopSettingsDialog({ onClose }: { onClose(): void }) {
  ...
  const [effective, setEffective] = useState<EffectiveDesktopSettings | null>(null)
  const [status, setStatus] = useState<
    'loading' | 'ready' | 'saving' | 'restarting' | 'error'
  >('loading')
  ...
  function updateSetting<K extends keyof DesktopSettings>(
    name: K,
  ...
  function handleProviderChange(event: ChangeEvent<HTMLSelectElement>) {
    const provider = event.target.value as DesktopAgentProvider
