...
function cardTitle(card, fallback = 'Unknown card') {
  return card
    ? [card.name, card.subtitle].filter(Boolean).join(' — ')
    : fallback
}

function compareCardItems(left, right) {
  return (
...
function CollectionProgressRing({ active, progress, onSelect }) {
...
function CollectionSummary({ analysis, activeSet, onSelectSet }) {
  return (
...
function revealCollectionImage(event) {
  event.currentTarget.classList.add('is-loaded')
}

function CollectionCardArtwork({ card, count, exactCountLabel, title }) {
...
function CollectionCardTile({ item, mode, onSetCount }) {
...
function emptyMessage(mode, query) {
...
export function CardCollectionDialog({
  ...
  const searchedItems = useMemo(() => {
    ...
    const matched = fuzzySearchCards(localSearchIndex, trimmedQuery, visibleCount + 1)
      .map((card) => itemById.get(getCatalogCardId(card)))
      .filter(Boolean)
...
export default function CardCollectionControl(props) {
