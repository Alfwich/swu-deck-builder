...
test('Electron Forge declares native makers for Windows, macOS, and Linux', () => {
  ...
  assert.equal(forgeConfig.packagerConfig.icon, './src/desktop/assets/icon')
  assert.equal(
  ...
  assert.equal(squirrelConfig.setupIcon, './src/desktop/assets/icon.ico')
  assert.equal(
  ...
  assert.equal(
    forgeConfig.packagerConfig.ignore.some((pattern) =>
      pattern.test('/src/shared/google-drive-api.ts'),
    ),
    true,
  )
  assert.equal(
    forgeConfig.packagerConfig.ignore.some((pattern) =>
      pattern.test('/.runtime/shared/google-drive-api.js'),
    ),
    false,
  )
})
