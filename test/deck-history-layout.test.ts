...
test('deck history stays above the central deck workspace', async () => {
  const app = await readFile(new URL('../src/web/decks/deck-workspace.tsx', import.meta.url), 'utf8')
...
test('deck history renders only its timeline controls', async () => {
  const component = await readFile(
    new URL('../src/web/decks/deck-history-bar.tsx', import.meta.url),
    'utf8',
  )
...
test('stacked deck history entries use the card-change review dialog', async () => {
  const [app, workspace, css] = await Promise.all([
    ...
    readFile(new URL('../src/web/decks/deck-workspace.tsx', import.meta.url), 'utf8'),
    readStyles(),
  ])
  const source = `${app}\n${workspace}`
...
test('branching history requires explicit destructive confirmation', async () => {
  const [app, dialog, historyDiscard, css] = await Promise.all([
    readFile(new URL('../src/web/app/app.tsx', import.meta.url), 'utf8'),
    readFile(new URL('../src/web/decks/deck-library.tsx', import.meta.url), 'utf8'),
    readFile(new URL('../src/web/decks/use-history-discard.ts', import.meta.url), 'utf8'),
    readStyles(),
  ])
  const source = `${app}\n${dialog}\n${historyDiscard}`
