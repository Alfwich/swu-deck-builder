...
const expectedPngs = new Map([
  ...
  ['../desktop/assets/icon.png', 512],
])

test('the authored icon source is a large transparent PNG', async () => {
  const metadata = await sharp(
    fileURLToPath(new URL('../src/assets/app-icon.png', import.meta.url)),
  ).metadata()
...
test('macOS icon includes native and Retina PNG representations', async () => {
  const icns = await readFile(new URL('../desktop/assets/icon.icns', import.meta.url))
